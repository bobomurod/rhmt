...
// Также надо написать тесты - много тестов по разным трансферам
// c проверкой переполнения буфера и минусовых балансов
    ...
    var fromBalance = Holder.findOne({wallet: req.body.from}, function (err, fromHolder) {
        ...
        if (fromHolder.balance >= req.body.value) {
            ...
            Holder.findOne({wallet: req.body.to}, function(err, holder){
                        if (err) throw err;
                        _toBalance = holder.balance + parseInt(req.body.value);
